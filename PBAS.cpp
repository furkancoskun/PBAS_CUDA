//Copyright: Philipp Tiefenbacher, Martin Hofmann
...
bool PBAS::process(cv::Mat* input, cv::Mat* output)
{
	...
	std::cout << "CPU - totalNorm: " << maxNorm << std::endl;

	double meanNorm = maxNorm/((double)(glCounterFore+1));

	std::cout << "CPU - totalCounter: " << (glCounterFore+1) << std::endl;
	std::cout << "CPU - NORM: " << meanNorm << std::endl << std::endl;
